...
export default function RestuarnatAllFood({ CategoryWise }) {
    const allRestaurants = useSelector((state) => state.resturant.allRestaurants);
        ...
        <div className="max-w-[80%] min-w-[80%] justify-center mx-auto container">
            ...
            <div className="flex gap-3 rounded-2xl shadow-orange-100 border-amber-50 justify-center overflow-x-auto">
                {CategoryWise?.filter(
                    ...
                    <div className="bg-amber-500 rounded-2xl px-2 py-2 whitespace-nowrap" key={index}>
                        {item?.facetInfo?.[0]?.label}
                    </div>
                ))}
            </div>
            ...
            <div className="mt-5 gap-8 justify-center flex flex-wrap">
                {sortedRestaurants?.map((data) => (
                    <Link
                        to={`/city/delhi/${data?.info?.id}`}
                        key={data.info.id}
                        className="h-[248px] cursor-pointer block transform mb-5 hover:scale-95 transition duration-50 ease-in-out"
                    >
                        ...
                        <div className="h-[92px] w-[92.297px] ml-3 mt-1">
                            ...
                            <div className="flex text-[16px] w-[261px] text-[#02060ceb]">
                                <div className="flex justify-around items-center">
                                    ...
                                    <span className="text-xl font-bold flex justify-center min-w-[15px] leading-none">•</span>
                                </div>
