...
export default function RestuarnatAllFood({ CategoryWise, ResturantAllFood }) {
  return (
    <div className=" w-[80%] justify-center mx-auto container">
      <div className="">
        <h2>Resturant with online food delivery </h2>
      </div>

      <div className="flex gap-3  rounded-2xl shadow-orange-100 border-amber-50 justify-center">
             
        {
          CategoryWise?.filter( (data) => data?.facetInfo?.[0]?.label !== "Afghani")
          .map((item, index) => (<div className=" bg-amber-500 rounded-2xl px-2 py-2" key={index}>{item?.facetInfo?.[0]?.label}</div>)
        )}
      </div>
        ...
        {ResturantAllFood?.map((data) => (
          <Link
            ...
            <div className="relative">
              ...
              <div
                className="absolute bottom-0 left-0 w-full  h-[70px] font-extrabold z-10
