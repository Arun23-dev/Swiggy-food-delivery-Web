...
const SectionHeader = ({ title, label, onClick }) => (
  <div className="flex items-center justify-between mb-4">
    ...
    <button onClick={onClick} className="text-orange-500 text-sm font-medium hover:underline flex items-center gap-1">
...
export default function Dashboard() {
  ...
  // Compute cart totals based on ALL cart items
  const totalCartCount = cartItems.reduce((sum, item) => sum + item.quantity, 0);
  const cartSubtotal = cartItems.reduce(
    (sum, item) => sum + Math.floor((item.defaultPrice || item.price || 0) / 100) * item.quantity,
    0
  );
  const cartDelivery = cartSubtotal >= 500 ? 0 : (cartItems.length > 0 ? 30 : 0);
  ...
  const kpis = [
    {
      ...
      sub: totalCartCount > 0 ? `₹${cartTotal} ready to checkout` : 'Cart is empty',
      icon: ShoppingCart,
  ...
  return (
    <div className="w-full min-h-full bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        {/* Cart Preview */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <SectionHeader title="Your Cart" label="" onClick={() => navigate('/checkout')} />
          </div>
          {cartItems.length === 0 ? (
            ...
            <div className="p-5 space-y-4">
              {recentItems?.map((item) => (
                <div key={`${item.restaurantId}_${item.swiggyItemId}`} className="flex items-center gap-3">
                  <img
              ...
              <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                ...
                <Button className="bg-orange-400 hover:bg-orange-600 text-white w-80 py-8 text-lg font-bold rounded-xl" onClick={() => navigate('/checkout')}>
                  Checkout
                </Button>
              </div>
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <SectionHeader title="Your Order" label="View Orders" onClick={() => navigate("/dashboard/orders")} />
          </div>
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {recentOrders?.map((order) => {
                ...
                const firstItem = firstRestaurant?.items?.[0];
                ...
                return (
                  <div key={order?._id} className="p-4 hover:bg-gray-50 transition cursor-pointer">
                    <div className="flex gap-3">
                      <img
                        src={`${SWIGGY_IMAGE_BASE_URL}/${itemImage}`}
                        alt={itemName}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <p className="font-semibold text-sm text-gray-800">Order #{order?._id.slice(-6)}</p>
                          <StatusBadge status={order?.status} />
                        </div>
                        <p className="text-xs text-gray-400 mb-1">{new Date(order?.createdAt).toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-6">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 + i * 0.07 }} className={`${bg} rounded-2xl p-5 flex items-start gap-4`}>
