...
export default function RestuarnatAllFood({ CategoryWise, ResturantAllFood }) {
  ...
  return (
    <div className=" w-[80%] justify-center mx-auto container">
      <div className="">
        <h2>Resturant with onlice food delivery </h2>
      </div>
        ...
        {ResturantAllFood?.length > 0 ? (
          ResturantAllFood.map((data) => (
            <a
              key={data.info.id}
              href={data?.cta?.link ?? "#"}
              className=" h-[248pxb] cursor-pointer  block transform  hover:scale-95 transition duration-50 ease-in-out"
            >
              <div className="relative">
                ...
                {/* Gradient + Header/SubHeader */}
                              <div className="absolute bottom-0 left-0 w-full  h-[70px] font-extrabold z-10
